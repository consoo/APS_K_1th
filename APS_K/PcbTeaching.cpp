...
void CMotorDlg2::InitGridCtrl_Pcb()
{
   ...
   // m_clGridTeachingPcb.SetItemText(14, 0, MotorPosName[OC_2800K_Pos]);
	...
	m_clGridTeachingPcb.SetItemText(0, 7, tmpStr);
#endif
...
void CMotorDlg2::ShowGridCtrl_Pcb()
{
	...
	getMotorPos(Motor_Insp_X, Wait_Pos, pos, true);
	posStr.Format("%0.3f", pos);
	m_clGridTeachingPcb.SetItemText(7, 7, posStr);

	getMotorPos(Motor_Insp_X, Loading_Pos, pos, true);
	posStr.Format("%0.3f", pos);
	m_clGridTeachingPcb.SetItemText(8, 7, posStr);

	getMotorPos(Motor_Insp_X, Holder_Pos, pos, true);
	posStr.Format("%0.3f", pos);
	m_clGridTeachingPcb.SetItemText(9, 7, posStr);

	getMotorPos(Motor_Insp_X, Align_Pos, pos, true);
	posStr.Format("%0.3f", pos);
	m_clGridTeachingPcb.SetItemText(10, 7, posStr);

	getMotorPos(Motor_Insp_X, Bonding_Pos, pos, true);
	posStr.Format("%0.3f", pos);
	m_clGridTeachingPcb.SetItemText(11, 7, posStr);

	getMotorPos(Motor_Insp_X, Dark_Pos, pos, true);
	posStr.Format("%0.3f", pos);
	m_clGridTeachingPcb.SetItemText(12, 7, posStr);

	getMotorPos(Motor_Insp_X, OC_6500K_Pos, pos, true);
	posStr.Format("%0.3f", pos);
	m_clGridTeachingPcb.SetItemText(13, 7, posStr);

#if (____AA_WAY == PCB_TILT_AA)

#elif (____AA_WAY == LENS_TILT_AA)
	getMotorPos(Motor_Insp_X, Lens_Pos, pos, true);
	posStr.Format("%0.3f", pos);
	m_clGridTeachingPcb.SetItemText(14, 7, posStr);
#endif
...
void CMotorDlg2::MotorVellimitView()
{
	...
	if(m_iVel_Limit ==1)
	{
...
#if (__MACHINE_MODEL == MACHINE_1ST)
		for (i = Motor_PCB_X; i <= Motor_PCB_Xt; i++)
#else
		for (i = Motor_PCB_X; i <= Motor_PCB_Yt; i++)
#endif
		...
		m_clGridTeachingPcb.SetItemText(5, 7, temp);

		temp.Format("%0.3f", model.axis[Motor_Insp_X].m_dLimit_Err);
		m_clGridTeachingPcb.SetItemText(6, 7, temp);
#endif
	}
	else	// 속도 가속도
	{
...
void CMotorDlg2::dispMotorStatus()
{
	...
	m_clGridTeachingPcb.SetItemText(pcbRow - 1, 7, szLog);

	if (motor.GetAmpFault(Motor_Insp_X))
	{
		m_clGridTeachingPcb.SetItemBkColor(1, (7), GRID_COLOR_RED);
	...
	{
		m_clGridTeachingPcb.SetItemBkColor(1, (7), GRID_COLOR_WHITE);
	}

	if (motor.GetNegaSensor(Motor_Insp_X))
	{
		m_clGridTeachingPcb.SetItemBkColor(2, (7), GRID_COLOR_RED);
	...
	{
		m_clGridTeachingPcb.SetItemBkColor(2, (7), GRID_COLOR_WHITE);
	}

	if (motor.GetHomeSensor(Motor_Insp_X))
		m_clGridTeachingPcb.SetItemBkColor(3, (7), GRID_COLOR_GREEN);
	else
		m_clGridTeachingPcb.SetItemBkColor(3, (7), GRID_COLOR_WHITE);

	if (motor.GetPosiSensor(Motor_Insp_X))
	{
		m_clGridTeachingPcb.SetItemBkColor(4, (7), GRID_COLOR_RED);
		_stprintf_s(szLog, SIZE_OF_1K, _T("[%s] + Limit 감지"), MotorName[Motor_Insp_X]);
		if (!delayDlg->IsWindowVisible())				delayMsg(szLog, 3000, M_COLOR_RED);
	}
	else
	{
		m_clGridTeachingPcb.SetItemBkColor(4, (7), GRID_COLOR_WHITE);
	}
#endif
	m_clGridTeachingPcb.Invalidate();
}
...
void CMotorDlg2::OnBnClickedButtonMotorPosSave()
{
	...
	
	{
		...
		if(m_iVel_Limit == 1)
		{
			...
			//Motor_Insp_X
			cellStr = m_clGridTeachingPcb.GetItemText(5, 7);
			model.axis[Motor_Insp_X].m_dLimit_OK = (float)atof(cellStr);

			cellStr = m_clGridTeachingPcb.GetItemText(6, 7);
	...
	//
	////////////////////////////////////////////////////////////////////////////////////
	cellStr = m_clGridTeachingPcb.GetItemText(7, 7);
	model.axis[Motor_Insp_X].pos[Wait_Pos] = (atof(cellStr) + sysData.dDataset[Motor_Insp_X]);

	cellStr = m_clGridTeachingPcb.GetItemText(8, 7);
	model.axis[Motor_Insp_X].pos[Loading_Pos] = (atof(cellStr) + sysData.dDataset[Motor_Insp_X]);

	cellStr = m_clGridTeachingPcb.GetItemText(9, 7);
	model.axis[Motor_Insp_X].pos[Holder_Pos] = (atof(cellStr) + sysData.dDataset[Motor_Insp_X]);

	cellStr = m_clGridTeachingPcb.GetItemText(10, 7);
	model.axis[Motor_Insp_X].pos[Align_Pos] = (atof(cellStr) + sysData.dDataset[Motor_Insp_X]);

	cellStr = m_clGridTeachingPcb.GetItemText(11, 7);
	model.axis[Motor_Insp_X].pos[Bonding_Pos] = (atof(cellStr) + sysData.dDataset[Motor_Insp_X]);

	cellStr = m_clGridTeachingPcb.GetItemText(12, 7);
	model.axis[Motor_Insp_X].pos[Dark_Pos] = (atof(cellStr) + sysData.dDataset[Motor_Insp_X]);

	cellStr = m_clGridTeachingPcb.GetItemText(13, 7);
	model.axis[Motor_Insp_X].pos[OC_6500K_Pos] = (atof(cellStr) + sysData.dDataset[Motor_Insp_X]);

#if (____AA_WAY == PCB_TILT_AA)

#elif (____AA_WAY == LENS_TILT_AA)
	cellStr = m_clGridTeachingPcb.GetItemText(14, 7);
	model.axis[Motor_Insp_X].pos[Lens_Pos] = (atof(cellStr) + sysData.dDataset[Motor_Insp_X]);
	/*cellStr = m_clGridTeachingPcb.GetItemText(15, iIndex + 1);
	model.axis[i].pos[Pcb_Loading_Pos] = (atof(cellStr) + sysData.dDataset[i]);
	cellStr = m_clGridTeachingPcb.GetItemText(16, iIndex + 1);
	model.axis[i].pos[Cover_Loading_Pos] = (atof(cellStr) + sysData.dDataset[i]);
