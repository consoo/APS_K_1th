...
int CPcbProcess2::procProductComplete(int iStep)
{
	...
	case 12000:
		...
		iRtnFunction = 12800;// 12900;			//laser º¯À§·Î Á¡ÇÁ
		break;
...
int CPcbProcess2::RunProc_LENS_AlignLaserMeasure(int iLensStep)
{
	...
	case 50900:
		...
		{
			iRtnFunction = 51100;			//Laser ÃøÁ¤ Pass
		}
		else
		{
			iRtnFunction = 51250; //51000;		// 31300;//¾ó¶óÀÎ ÀÓ½Ã ÆÐ½º
		}
	...
	case 51009:
		...
		{
			Task.LensTaskTime = myTimer(true);
			logStr.Format("Lens ´ë±â À§Ä¡ ÀÌµ¿ ¿Ï·á[%d]", iLensStep);
			putListLog(logStr);
			iRtnFunction = 51010;
		}
		else
		{
			logStr.Format("Lens ´ë±â À§Ä¡ ÀÌµ¿ ½ÇÆÐ[%d]", iLensStep);
			errMsg2(Task.AutoFlag, logStr);
			iRtnFunction = -51009;
		}
		break;
	case 51010:
		...
		{
			Task.PCBTaskTime = myTimer(true);
			logStr.Format("Insp X Lens Align  À§Ä¡ ÀÌµ¿ ¿Ï·á[%d]", iLensStep);
			putListLog(logStr);
			iRtnFunction = 51050;
		}
		else
		{
			Task.PCBTaskTime = myTimer(true);
			logStr.Format("Insp X Lens Align  À§Ä¡ ÀÌµ¿ ½ÇÆÐ[%d]", iLensStep);
			putListLog(logStr);
			iRtnFunction = -51010;
		}
		break;
	...
	case 51100: // lens Align
		...
		else
		{
			...
			putListLog(logStr);
			iRtnFunction = 51100;					//  Àç°Ë»ç 
		}
		break;

	case 51200://!! º¸Á¤·® ÀÌµ¿.- Retry
	...
	case 51260:
		...
		{
			logStr.Format("Insp X Ãà Laser_Lens_Pos ÀÌµ¿ ½ÇÆÐ.[%d]", iLensStep);
			errMsg2(Task.AutoFlag, logStr);
			iRtnFunction = -51260;
		}
	case 51300://! Lens-ZÃà ´ë±â À§Ä¡ ÀÌµ¿
		iLaser_Pos = 0;
		...
		break;
	case 60000://! Laser-ZÃà ¿ÜºÎ ÃøÁ¤À§Ä¡ ÀÌµ¿
	...
	case 60200://! Lens Stage ¿ÜºÎ Laser ÃøÁ¤À§Ä¡ ÀÌµ¿ 
		if (motor.Lens_Motor_Move_Laser(iLaser_Pos, Task.d_Align_offset_x[LENS_Align_MARK], Task.d_Align_offset_y[LENS_Align_MARK], Task.d_Align_offset_th[LENS_Align_MARK]))
		{
			Task.LensTaskTime = myTimer(true);
			logStr.Format("Laser º¯À§ ÃøÁ¤ ¿ÜºÎ À§Ä¡ ÀÌµ¿ ¿Ï·á[%d]", iLensStep);	//Laser º¯À§ ÃøÁ¤ ¿ÜºÎ À§Ä¡ ÀÌµ¿ ¿Ï·á[%d]
			putListLog(logStr);
			if (iLaser_Pos == 0) { iRtnFunction = 60210; }//Ã³À½¸¸ ÇÏ°­¸í·É.
	...
	case 61000: //¿ÜºÎ º¯À§ °¨Áö (ÀúÀå µû·Î ÇÏ°í Align ÀÌµ¿)
	...
	case 61050: // ÀÌµ¿  º¯À§°ªÀÌ 0À¸·Î ¸¸µé±â À§ÇØ ÀÌµ¿
	...
	case 61200://! Lens-ZÃà ´ë±â À§Ä¡ ÀÌµ¿
		if (motor.LENS_Z_Motor_Move(Wait_Pos))
		{
			Task.LensTaskTime = myTimer(true);
			iRtnFunction = 61850;;//// 41300; //41850;//ÀÓ½Ã ·»Áî align pass
								  //iRtnFunction = 41300;
		}
		else
		{
	...
	case 61600://! LENS Align(º¯À§ÃøÁ¤ º¸Á¤ÈÄ, Shift °è»ê ¿ëµµ)
		...
		else
		{
			...
			iRtnFunction = 61600;					//  Àç°Ë»ç 
		}
		break;

	case 61700://!! º¸Á¤·® ÀÌµ¿.- Retry
		if (motor.Lens_Motor_Align_Move(Task.d_Align_offset_x[LENS_Align_MARK], Task.d_Align_offset_y[LENS_Align_MARK], Task.d_Align_offset_th[LENS_Align_MARK]))
		{
			Task.LensTaskTime = myTimer(true);
			iRtnFunction = 61500;
	...
	case 61870:
		if (motor.INSP_Motor_MoveX(Wait_Pos))
		{
			logStr.Format("Insp X ´ë±âÀ§Ä¡ ÀÌµ¿ ¸í·É[%d]", iLensStep);
			putListLog(logStr);
			iRtnFunction = 61900;		
		}
		else
		{
			logStr.Format("Insp X ´ë±âÀ§Ä¡ ÀÌµ¿ ½ÇÆÐ[%d]", iLensStep);
			putListLog(logStr);
			iRtnFunction = -61870;
		}
		break;
	case 61900://! Lens-ZÃà ´ë±â À§Ä¡ ÀÌµ¿
		if (motor.LENS_Z_Motor_Move(Wait_Pos))
		{
			Task.LensTaskTime = myTimer(true);
			iRtnFunction = 61950;
