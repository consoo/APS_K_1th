...
////#if (____MODEL_NAME == M_COB_8M || ____MODEL_NAME ==  M_TRINITY_WIDE ||____MODEL_NAME == M_TRINITY ||____MODEL_NAME == M_DANI||____MODEL_NAME == M_GEN2)		//도포 위치 이동 버튼
...
void CPcbDlg::changeColorMotorPosBtn()
{
	...
	else if (checkCurInspXMotorPos(Holder_Pos))					iIndex = 2;
	else if (checkCurInspXMotorPos(Dispense_Pos))				iIndex = 3;
	else if (checkCurInspXMotorPos(Dispense_Pos2))				iIndex = 4;
	else if (checkCurInspXMotorPos(Calc_Pos))					iIndex = 5;
	else if (checkCurInspXMotorPos(Align_Pos))					iIndex = 6;
	else if (checkCurInspXMotorPos(CompleteAlign_Pos))			iIndex = 7;
	else if (checkCurInspXMotorPos(Clean_Pos))					iIndex = 10;
	else if (checkCurInspXMotorPos(Com_Laser_Pos))				iIndex = 11;
	else if (checkCurInspXMotorPos(PDispense_Pos))				iIndex = 12;

	m_btnInspXWaitPos.m_iStateBtn = 0;
	m_btnInspXHolderPos.m_iStateBtn = 0;
	m_btnInspXBondPos.m_iStateBtn = 0;
	m_btnInspXPDispensePos.m_iStateBtn = 0;
	m_btnInspXTDispensePos.m_iStateBtn = 0;

	if (iIndex == 0)			m_btnInspXWaitPos.m_iStateBtn = 3;
	else if (iIndex == 2)		m_btnInspXHolderPos.m_iStateBtn = 3;
	else if (iIndex == 3)		m_btnInspXBondPos.m_iStateBtn = 3;
	else if (iIndex == 12)		m_btnInspXPDispensePos.m_iStateBtn = 3;
	else if (iIndex == 4)		m_btnInspXTDispensePos.m_iStateBtn = 3;

	m_btnInspXWaitPos.Invalidate();
	m_btnInspXHolderPos.Invalidate();
	m_btnInspXBondPos.Invalidate();
	m_btnInspXPDispensePos.Invalidate();
	m_btnInspXTDispensePos.Invalidate();
#endif
}
...
void CPcbDlg::OnBnClickedButtonEpoxy()
{
	...
	if (!checkCurInspXMotorPos(Dispense_Pos))
	{
		logStr.Format("INSP X 축 Epoxy 기준위치 이동 후 진행하세요.");
...
void CPcbDlg::OnBnClickedButtonMotorLaserOut1()
{
	...
	if (!motor.INSP_Motor_MoveX(pMotorPos))
	{
		sLangChange.LoadStringA(IDS_STRING1499);	//PCB부 X, Y, Theta축 대기 위치 이동 실패
		sTmpLog.Format(sLangChange, MotorPosName[pMotorPos]);
		delayMsg(sTmpLog.GetBuffer(99), 3000, M_COLOR_GREEN);
		g_bMovingflag = false;
		return;
	}
...
void CPcbDlg::OnBnClickedButtonInspXMotorBond()
{
	...
	{
		sLangChange.LoadStringA(IDS_STRING1499);	//"PCB %s 이동 실패"
		sTmpLog.Format(sLangChange, MotorPosName[Dispense_Pos]);
		delayMsg(sTmpLog.GetBuffer(99), 3000, M_COLOR_RED);
	}
	sLangChange.LoadStringA(IDS_STRING1498);	//"PCB %s 이동 완료"
...
void CPcbDlg::OnBnClickedButtonInspXMotorPdispensePos()
{
	...
	if (!motor.INSP_Motor_MoveX(PDispense_Pos))//,0.0,0.0,0.0))
	{
		sLangChange.LoadStringA(IDS_STRING1499);	//"PCB %s 이동 실패"
		sTmpLog.Format(sLangChange, MotorPosName[PDispense_Pos]);
		delayMsg(sTmpLog.GetBuffer(99), 3000, M_COLOR_RED);
...
void CPcbDlg::OnBnClickedButtonInspXMotorTdispensePos()
{
	...
	if (!motor.INSP_Motor_MoveX(Dispense_Pos2))//,0.0,0.0,0.0))
	{
