...
//#endif // _DEBUG
	...
	if (lNumberOfDigitizer>0)
	{
		...
		MdigAlloc(MilSystem, M_DEFAULT, "DCF1st.dcf", M_DEFAULT, &MilDigitizer);
#else
		MdigAlloc(MilSystem,M_DEFAULT,"DCF_APSK2.dcf",M_DEFAULT,&MilDigitizer);
#endif
...
bool CVision::FnShmEdgeFind(BYTE* ChartRawImage, bool mAutoMode)//CDPoint& clDLCenter, CDPoint& clDRCenter, 
{
	int i = 0;
	int j = 0;
	TCHAR szLog[SIZE_OF_1K];

	//clDLCenter.x = clDLCenter.y = 0.0;
	//clDRCenter.x = clDRCenter.y = 0.0;
	TDATASPEC tspec;
	...
	ACMISSoftISP::xMakeBMP(ChartRawImage, (byte*)m_pBMPTempBuffer, gMIUDevice.nWidth, gMIUDevice.nHeight, tspec);
	...
	int mRoiX = 0;// 520;// 500;// centX - 850;		//2592
	int mRoiY = 0;// centY - (mRoiHeight / 2);// +100;

	int mRoiWidth = gMIUDevice.nWidth - mRoiX;//1680;
	int mRoiHeight = gMIUDevice.nHeight - mRoiY;// 400;
	...
	{ 
		if (!contours[i].empty())
		{
			...
			if (mcircleSize > minSize && mcircleSize < maxSize && boundingRectData.width < boundWidMax)//300)//mcircleSize > minSize && mcircleSize < maxSize)
			{
				...
				int nIndex = sfrBoundCheck(cx, cy);
				//nIndex > 0  && 
				if (nIndex > 0 && cy > yLimit && cx > xLimit && cx < gMIUDevice.nWidth - xLimit&& cy < gMIUDevice.nHeight - yLimit)		//좌측 빛샘 때문에 100 추가
				{
					...
					for (j = 0; j < approx.size(); j++) 
					{
						MandoInspLog.m_ChartVertex[nIndex].Pos[j].x = mPoint[j].x;		//0 = LT , 1 = BL = 2 = RT , 3 = BR
						MandoInspLog.m_ChartVertex[nIndex].Pos[j].y = mPoint[j].y;
						//if (nIndex == 0)
						//{
						//	if (bCenterCheck == false)
						//	{
						//		if (j == 3)
						//		{
						//			bCenterCheck = true;
						//		}
						//		MandoInspLog.m_ChartVertex[1].Pos[j].x = mPoint[j].x;		//0 = LT , 1 = BL = 2 = RT , 3 = BR
						//		MandoInspLog.m_ChartVertex[1].Pos[j].y = mPoint[j].y;
						//	}
						//	else
						//	{
						//		MandoInspLog.m_ChartVertex[0].Pos[j].x = mPoint[j].x;		//0 = LT , 1 = BL = 2 = RT , 3 = BR
						//		MandoInspLog.m_ChartVertex[0].Pos[j].y = mPoint[j].y;
						//	}
						//	
						//}
						//else
						//{
						//	MandoInspLog.m_ChartVertex[nIndex + 1].Pos[j].x = mPoint[j].x;		//0 = LT , 1 = BL = 2 = RT , 3 = BR
						//	MandoInspLog.m_ChartVertex[nIndex + 1].Pos[j].y = mPoint[j].y;
						//}
						

						//circle(GaussianImage, approx[i], 5 + (i * 2), Scalar(255, 255, 255), -1); // 꼭짓점에 원 그리기
						
						if (j == 0)
						{
							mCircleColor = Scalar(0, 255, 0);
						}
						else
						{
							colordiff = (j * 35);
							mCircleColor = Scalar(255 - colordiff, 255 - colordiff, 255 - colordiff);
							
						}
						circle(OrgImage, mPoint[j], 5 + (j * 1), mCircleColor, -1); // 꼭짓점에 원 그리기
	...
	if (nFindCount < 1)//16)//if (nFindCount < 12) 
	{
		_stprintf_s(szLog, SIZE_OF_1K, _T("[Vertex]Find Fail"));
		theApp.MainDlg->putListLog(szLog); 
	...
	{
		chartNum = i;
		count1 = model.mCurModelName.Find(_T("100"));
		count2 = model.mCurModelName.ReverseFind('100');
		if (count2 - count1 > 1 && count1 > 0)
		{
			if (i == 1)
			{
				_stprintf_s(szLog, SIZE_OF_1K, _T("[Chart %d]LT-x:%d/y:%d"), chartNum, MandoInspLog.m_ChartVertex[i].Pos[1].x, MandoInspLog.m_ChartVertex[i].Pos[1].y);
				theApp.MainDlg->putListLog(szLog);
				_stprintf_s(szLog, SIZE_OF_1K, _T("[Chart %d]LT-x:%d/y:%d"), chartNum, MandoInspLog.m_ChartVertex[i].Pos[7].x, MandoInspLog.m_ChartVertex[i].Pos[7].y);
				theApp.MainDlg->putListLog(szLog);
			}
			else if (i == 2)
			{
				_stprintf_s(szLog, SIZE_OF_1K, _T("[Chart %d]LT-x:%d/y:%d"), chartNum, MandoInspLog.m_ChartVertex[i].Pos[0].x, MandoInspLog.m_ChartVertex[i].Pos[0].y);
				theApp.MainDlg->putListLog(szLog);
				_stprintf_s(szLog, SIZE_OF_1K, _T("[Chart %d]LT-x:%d/y:%d"), chartNum, MandoInspLog.m_ChartVertex[i].Pos[7].x, MandoInspLog.m_ChartVertex[i].Pos[7].y);
				theApp.MainDlg->putListLog(szLog);
			}
			...
			{
				_stprintf_s(szLog, SIZE_OF_1K, _T("[Chart %d]LT-x:%d/y:%d"), chartNum, MandoInspLog.m_ChartVertex[i].Pos[2].x, MandoInspLog.m_ChartVertex[i].Pos[2].y);
				theApp.MainDlg->putListLog(szLog);
			}
			else if (i == 7)
			{
			...
			{
				_stprintf_s(szLog, SIZE_OF_1K, _T("[Chart %d]LT-x:%d/y:%d"), chartNum, MandoInspLog.m_ChartVertex[i].Pos[3].x, MandoInspLog.m_ChartVertex[i].Pos[3].y);
				theApp.MainDlg->putListLog(szLog);
			}
			else if (i == 12)
			{
		...
		//sprintf_s(strTmp, "[x:%d,y:%d]", LTposx, LTposy);
		//putText(OrgImage, strTmp, Point(LTposx - 100, LTposy-30), myFontFace, myFontScale, Scalar(100, 255, 200), mthickness);
		
	}
	if (mAutoMode == false)
	{
		// 윈도우 생성 및 이미지 표시
		namedWindow("OrgImage", WINDOW_NORMAL);
	...
	CString		fileName = _T("");
	jpgName.Format("%s\\%s_%s_%04d%02d%02d%02d%02d%02d.jpg", FolderName, Task.ChipID, _T("Chartvertex"), time.wYear, time.wMonth, time.wDay, time.wHour, time.wMinute, time.wSecond);

	imwrite((LPSTR)(LPCTSTR)jpgName, OrgImage);
#endif
	return true;
}
