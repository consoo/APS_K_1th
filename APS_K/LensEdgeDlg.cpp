...
void CLensEdgeDlg::setInterface()
{
	...
	GetDlgItem(IDC_BUTTON_EDGE_INSP_X_MOTOR_WAIT)->ShowWindow(SW_HIDE);
	GetDlgItem(IDC_BUTTON_EDGE_INSP_X_MOTOR_ALIGN)->ShowWindow(SW_HIDE);
...
void CLensEdgeDlg::OnBnClickedButtonMotorGiveUp()
{
	...
	if (!motor.Lens_Motor_MoveX(Wait_Pos))
	{
		sTmpLog.Format("Lens %s 이동 실패", MotorPosName[Wait_Pos]);
		delayMsg(sTmpLog.GetBuffer(99), 3000, M_COLOR_RED);
		g_bMovingflag = false;
		return;
	}
...
void CLensEdgeDlg::OnBnClickedButtonMotorContiPos()
{
	...
	if (!motor.Lens_Motor_MoveXY(1, ContiTailDelay_Pos))
	{
...
void CLensEdgeDlg::OnBnClickedButtonLensLaserPos1()
{
	...
	{
		sTmpLog.Format("INSP X 축 %s 이동 실패.", MotorPosName[pMotorPos]);
		delayMsg(sTmpLog.GetBuffer(99), 3000, M_COLOR_GREEN);
		g_bMovingflag = false;
		return;
	}
...
#if (__MACHINE_MODEL == MACHINE_1ST)
	if (!motor.Lens_Motor_MoveXY(0, Pcb_Loading_Pos))
#else
	if (!motor.Lens_Motor_MoveXY(1, Pcb_Loading_Pos))
#endif
...
#if (__MACHINE_MODEL == MACHINE_1ST)
	if (!motor.Lens_Motor_MoveXY(0, Cover_Loading_Pos))
#else
	if (!motor.Lens_Motor_MoveXY(1, Cover_Loading_Pos))
#endif
...
#if (__MACHINE_MODEL == MACHINE_1ST)
	if (!motor.Lens_Motor_MoveXY(0, Holder_Uv_Pos))
#else
	if (!motor.Lens_Motor_MoveXY(1, Holder_Uv_Pos))
#endif
...
void CLensEdgeDlg::OnBnClickedButtonEdgeInspXMotorAlign()
{
	...
	{
		sLangChange.LoadStringA(IDS_STRING1499);	//Lens %s 이동 실패
		sTmpLog.Format(sLangChange, MotorPosName[Lens_Pos]);
		delayMsg(sTmpLog.GetBuffer(99), 3000, M_COLOR_RED);
		g_bMovingflag = false;
		return;
