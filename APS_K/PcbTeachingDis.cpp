...
void CMotorDlg3::InitGridCtrl_Dispense()
{
...
#else
	for (int i = Motor_PCB_X; i <= Motor_PCB_Yt; i++)
#endif
	...
	tmpStr.Format("%s", MotorName[Motor_Insp_X]);
	m_clGridTeachingDispense.SetItemText(0, 7, tmpStr);
#endif
	//
	int i=0, j=0;
...
void CMotorDlg3::ShowGridCtrl_Dispense()
{
	...
	getMotorPos(Motor_Insp_X, Com_Laser_Pos, pos, true);
	posStr.Format("%0.3f", pos);
	m_clGridTeachingDispense.SetItemText(7, 7, posStr);

	getMotorPos(Motor_Insp_X, Dispense_Pos, pos, true);
	posStr.Format("%0.3f", pos);
	m_clGridTeachingDispense.SetItemText(8, 7, posStr);

	getMotorPos(Motor_Insp_X, Laser_Pcb_Pos, pos, true);
	posStr.Format("%0.3f", pos);
	m_clGridTeachingDispense.SetItemText(9, 7, posStr);

	getMotorPos(Motor_Insp_X, CompleteAlign_Pos, pos, true);
	posStr.Format("%0.3f", pos);
	m_clGridTeachingDispense.SetItemText(10, 7, posStr);

	getMotorPos(Motor_Insp_X, PDispense_Pos, pos, true);
	posStr.Format("%0.3f", pos);
	m_clGridTeachingDispense.SetItemText(11, 7, posStr);


	getMotorPos(Motor_Insp_X, Holder_Uv_Pos, pos, true);
	posStr.Format("%0.3f", pos);
	m_clGridTeachingDispense.SetItemText(12, 7, posStr);


	getMotorPos(Motor_Insp_X, Dispense_Pos1, pos, true);
	posStr.Format("%0.3f", pos);
	m_clGridTeachingDispense.SetItemText(13, 7, posStr);

	getMotorPos(Motor_Insp_X, Dispense_Pos2, pos, true);
	posStr.Format("%0.3f", pos);
	m_clGridTeachingDispense.SetItemText(14, 7, posStr);

	getMotorPos(Motor_Insp_X, Dispense_Pos3, pos, true);
	posStr.Format("%0.3f", pos);
	m_clGridTeachingDispense.SetItemText(15, 7, posStr);
#endif
	m_clGridTeachingDispense.Invalidate();
}
...
void CMotorDlg3::dispMotorStatus()
{
	...
	//
	////////////////////////////////////////////////////////////////////////////////////
	_stprintf_s(szLog, SIZE_OF_1K, _T("%.03f"), motor.GetEncoderPos_Disp(Motor_Insp_X));
	m_clGridTeachingDispense.SetItemText(DispenseRow - 1, 7, szLog);

	if (motor.GetAmpFault(Motor_Insp_X))
	{
		m_clGridTeachingDispense.SetItemBkColor(1, (7), GRID_COLOR_RED);

		if (!delayDlg->IsWindowVisible())
		{
			_stprintf_s(szLog, SIZE_OF_1K, _T("[%s] AMP 알람 감지"), MotorName[Motor_Insp_X]);
			delayMsg(szLog, 3000, M_COLOR_RED);
		}
	}
	else
	{
		m_clGridTeachingDispense.SetItemBkColor(1, (7), GRID_COLOR_WHITE);
	}

	if (motor.GetNegaSensor(Motor_Insp_X))
	{
		m_clGridTeachingDispense.SetItemBkColor(2, (7), GRID_COLOR_RED);

		if (!delayDlg->IsWindowVisible())
		{
			_stprintf_s(szLog, SIZE_OF_1K, _T("[%s] - Limit 감지"), MotorName[Motor_Insp_X]);
			delayMsg(szLog, 3000, M_COLOR_RED);
		}
	}
	else
	{
		m_clGridTeachingDispense.SetItemBkColor(2, (7), GRID_COLOR_WHITE);
	}

	if (motor.GetHomeSensor(Motor_Insp_X))
		m_clGridTeachingDispense.SetItemBkColor(3, (7), GRID_COLOR_GREEN);
	else
		m_clGridTeachingDispense.SetItemBkColor(3, (7), GRID_COLOR_WHITE);

	if (motor.GetPosiSensor(Motor_Insp_X))
	{
		m_clGridTeachingDispense.SetItemBkColor(4, (7), GRID_COLOR_RED);
		_stprintf_s(szLog, SIZE_OF_1K, _T("[%s] + Limit 감지"), MotorName[Motor_Insp_X]);
		if (!delayDlg->IsWindowVisible())				delayMsg(szLog, 3000, M_COLOR_RED);
	}
	else
	{
		m_clGridTeachingDispense.SetItemBkColor(4, (7), GRID_COLOR_WHITE);
	}
#endif
	m_clGridTeachingDispense.Invalidate();
}
...
void CMotorDlg3::OnBnClickedButtonMotorPosSave()
{
	...
	if (m_iVel_Limit == 1)
	{
		cellStr = m_clGridTeachingDispense.GetItemText(5, 7);
		model.axis[Motor_Insp_X].m_dLimit_OK = (float)atof(cellStr);

		cellStr = m_clGridTeachingDispense.GetItemText(6, 7);
		model.axis[Motor_Insp_X].m_dLimit_Err = (float)atof(cellStr);

	}
	else
	{
		cellStr = m_clGridTeachingDispense.GetItemText(5, 7);
		sysData.fMotorSpeed[Motor_Insp_X] = (float)atof(cellStr);

		cellStr = m_clGridTeachingDispense.GetItemText(6, 7);
		sysData.fMotorAccTime[Motor_Insp_X] = (float)atof(cellStr);
	}


	cellStr = m_clGridTeachingDispense.GetItemText(7, 7);
	model.axis[Motor_Insp_X].pos[Com_Laser_Pos] = (atof(cellStr) + sysData.dDataset[Motor_Insp_X]);

	cellStr = m_clGridTeachingDispense.GetItemText(8, 7);
	model.axis[Motor_Insp_X].pos[Dispense_Pos] = (atof(cellStr) + sysData.dDataset[Motor_Insp_X]);

	cellStr = m_clGridTeachingDispense.GetItemText(9, 7);
	model.axis[Motor_Insp_X].pos[Laser_Pcb_Pos] = (atof(cellStr) + sysData.dDataset[Motor_Insp_X]);

	cellStr = m_clGridTeachingDispense.GetItemText(10, 7);
	model.axis[Motor_Insp_X].pos[CompleteAlign_Pos] = (atof(cellStr) + sysData.dDataset[Motor_Insp_X]);

	cellStr = m_clGridTeachingDispense.GetItemText(11, 7);
	model.axis[Motor_Insp_X].pos[PDispense_Pos] = (atof(cellStr) + sysData.dDataset[Motor_Insp_X]);

	cellStr = m_clGridTeachingDispense.GetItemText(12, 7);
	model.axis[Motor_Insp_X].pos[Holder_Uv_Pos] = (atof(cellStr) + sysData.dDataset[Motor_Insp_X]);

	cellStr = m_clGridTeachingDispense.GetItemText(13, 7);
	model.axis[Motor_Insp_X].pos[Dispense_Pos1] = (atof(cellStr) + sysData.dDataset[Motor_Insp_X]);

	cellStr = m_clGridTeachingDispense.GetItemText(14, 7);
	model.axis[Motor_Insp_X].pos[Dispense_Pos2] = (atof(cellStr) + sysData.dDataset[Motor_Insp_X]);

	cellStr = m_clGridTeachingDispense.GetItemText(15, 7);
	model.axis[Motor_Insp_X].pos[Dispense_Pos3] = (atof(cellStr) + sysData.dDataset[Motor_Insp_X]);
#endif
	model.Save();
	sysData.Save();
...
void CMotorDlg3::MotorVellimitView()
{
	...
	if(m_iVel_Limit ==1)
	{
		...
		temp.Format("%0.3f", model.axis[Motor_Insp_X].m_dLimit_OK);
		m_clGridTeachingDispense.SetItemText(5, 7, temp);

		temp.Format("%0.3f", model.axis[Motor_Insp_X].m_dLimit_Err);
		m_clGridTeachingDispense.SetItemText(6, 7, temp);
#endif
	}
	else	// 속도 가속도
	{
		...
		temp.Format("%0.3f", sysData.fMotorSpeed[Motor_Insp_X]);
		m_clGridTeachingDispense.SetItemText(5, 7, temp);

		temp.Format("%.03f", sysData.fMotorAccTime[Motor_Insp_X]);
		m_clGridTeachingDispense.SetItemText(6, 7, temp);
#endif
	}
}
